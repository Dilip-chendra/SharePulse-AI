...
const PAGE_META: Record<string, { label: string; icon: React.ElementType }> = {
  'overview': { label: 'Executive Command Center', icon: LayoutDashboard },
  'diagnose': { label: 'Diagnostic Studio', icon: PieChart },
  'sow': { label: 'Diagnose · Share of Wallet', icon: PieChart },
  'migration': { label: 'Diagnose · Payment Rails', icon: ArrowRightLeft },
  'big-ticket': { label: 'Diagnose · Ticket Size', icon: ShoppingBag },
  'prime-reward': { label: 'Diagnose · Reward Leakage', icon: Gift },
  'attrition': { label: 'Diagnose · Silent Defectors', icon: Radar },
  'return-friction': { label: 'Diagnose · Return Friction', icon: RotateCcw },
  'customers': { label: 'Customer 360 Intelligence', icon: Users },
  'segmentation': { label: 'Customer Archetypes', icon: Shapes },
  'leakage': { label: 'Margin Bleed Index', icon: TrendingDown },
  'opportunities': { label: 'Opportunity Pipeline & Audience Builder', icon: Target },
  'opportunity': { label: 'Opportunity Pipeline & Audience Builder', icon: Target },
  'nba': { label: 'Next Best Action Decision Engine', icon: Sparkles },
  'strategy': { label: 'Action Economics & Strategy', icon: Activity },
  'actions': { label: 'Action Center Matrix', icon: Shield },
  'experiments': { label: 'Experiment Center & Lift Measurement', icon: FlaskConical },
  'data': { label: 'Dataset Ingestion & Quality Screen', icon: Database },
  'datasets': { label: 'Dataset Ingestion & Quality Screen', icon: Database },
  'connections': { label: 'Data Connections', icon: Database },
  'evidence': { label: 'Auditable Claims, Methodology & Limitations', icon: FileCheck },
  'governance': { label: 'Data Governance', icon: FileCheck },
  'audit': { label: 'Audit & Security', icon: Shield },
  'settings': { label: 'Settings & Environment', icon: Database },
